...
const ROUTE_ITEMS = [
  { id: 'Cloud',  title: 'Cloud Anthropic', sub: 'Default · ORYND-hosted' },
  { id: 'Claude', title: 'BYO Claude',      sub: 'Needs Anthropic key', provider: 'anthropic' },
  { id: 'OpenAI', title: 'BYO OpenAI',      sub: 'Needs OpenAI key', provider: 'openai' },
  { id: 'Gemini', title: 'BYO Gemini',      sub: 'Needs Gemini key', provider: 'gemini' },
  { id: 'Local',  title: 'Local model',     sub: 'Needs local model endpoint', provider: 'local' },
];

function normalizeRoute(route) {
  if (!route || route === 'ORYND' || route === 'ORYND Cloud') return 'Cloud';
  return route;
}

function routeIsReady(item, settings) {
  if (item.id === 'Cloud') return true;
  if (!settings) return false;
  if (item.id === 'Local') return Boolean(settings.local_model && settings.local_model.available);
  return Boolean(
    settings.model_key &&
    settings.model_key.provider === item.provider &&
    settings.model_key.has_effective_key
  );
}

function disabledRouteReason(item) {
  if (item.id === 'Local') return 'Start local model first';
  return 'Add key in Settings';
}

function Composer({ value = '', route = 'ORYND', sending = false, disabled = false, selection = null }) {
  const [menu, setMenu] = React.useState(null); // 'attach' | 'route' | null
  const [activeRoute, setActiveRoute] = React.useState(normalizeRoute(route));
  const [settings, setSettings] = React.useState(null);
  const wrapRef = React.useRef(null);
  React.useEffect(() => {
    setActiveRoute(normalizeRoute(route));
  }, [route]);
  React.useEffect(() => {
    fetch('/api/settings')
  ...
  return h('div', { className: 'tp-composer', ref: wrapRef },
    ...
    menu === 'route' && h('div', { className: 'tp-pop route' },
      ...
      ROUTE_ITEMS.map((it) => {
        const ready = routeIsReady(it, settings);
        return h('button', {
          className: 'tp-popitem' + (!ready ? ' disabled-route' : ''),
          key: it.id,
          disabled: !ready,
          title: ready ? it.sub : disabledRouteReason(it),
          onClick: () => {
            if (!ready) return;
            setActiveRoute(it.id);
            setMenu(null);
          },
        },
        h('span', { className: 'pmain' },
          h('span', { className: 'pt' }, it.title),
          h('span', { className: 'ps' }, ready ? it.sub : disabledRouteReason(it))),
    ...
    h('div', { className: 'tp-input-wrap' },
      ...
      h('textarea', { className: 'tp-input', rows: value ? 2 : 1, defaultValue: value, placeholder: selection ? 'Edit this selection…' : 'Describe a part, or attach a reference…', readOnly: true }),
      h('div', { className: 'tp-input-row' },
        ...
        h('button', { className: 'tp-route' + (menu === 'route' ? ' open' : ''), onClick: () => setMenu(menu === 'route' ? null : 'route') },
          h('span', { className: 'rdot' }), ROUTE_ITEMS.find((it) => it.id === activeRoute)?.title || activeRoute, h(I.Chevron, { size: 12 })),
        h('button', { className: 'tp-send', disabled: disabled || sending }, h(I.Send, { size: 17 })),
      ),
    ),
  );
}
...
window.CAD = {
  ...
  ResultCard, Banner, EmptyState, Composer, TL_STEPS,
};
