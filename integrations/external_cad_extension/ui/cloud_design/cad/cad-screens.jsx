            ...
            _sc('div', { className: 'tp-set-card' },
              _sc('div', { className: 'tp-set-row', style: { display: 'block' } },
                ...
                _sc('div', { className: 'tp-seg' },
                  _sc('button', { className: 'active' }, 'Cloud Anthropic'),
                  _sc('button', { disabled: true, title: 'BYO routes stay disabled until a key is saved' }, 'BYO key'),
                  _sc('button', { disabled: true, title: 'Start local model first' }, 'Local'),
                ),
              ),
              _sc('div', { className: 'tp-set-row', style: { display: 'block' } },
                _sc('div', { style: { display: 'flex', alignItems: 'center', gap: 12 } },
                  _sc('span', { className: 'tp-set-ic' }, _sc(SI.Key, { size: 16 })),
                  _sc('div', { className: 'tp-set-main' }, _sc('div', { className: 'tp-set-title' }, 'Anthropic API key'), _sc('div', { className: 'tp-set-sub' }, 'Optional BYO Claude route · stored through the local bridge/keychain')),
                ),
                _sc('label', { className: 'tp-keyfield tp-key-input' },
                  _sc(SI.Key, { size: 15, style: { color: 'var(--ink-3)' } }),
                  _sc('input', {
                    type: 'password',
                    placeholder: 'Paste sk-ant key to enable BYO Claude',
                    autoComplete: 'off',
                    readOnly: true,
                  }),
                  _sc('button', { className: 'tp-btn sm', type: 'button' }, 'Save'),
                ),
                _sc('div', { className: 'tp-set-sub', style: { marginTop: 8 } }, 'Cloud Anthropic works by default. BYO routes stay disabled until a key is saved.'),
              ),
              _sc('div', { className: 'tp-set-row', style: { display: 'block' } },
                _sc('div', { style: { display: 'flex', alignItems: 'center', gap: 12 } },
                  _sc('span', { className: 'tp-set-ic' }, _sc(SI.Key, { size: 16 })),
...
Object.assign(window.CAD, { SubHeader, SettingsScreen, Toast, SettingsSwitch: Switch });
